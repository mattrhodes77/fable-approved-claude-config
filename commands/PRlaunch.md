...
1. **Write the scenarios and PASS criteria BEFORE running anything.** One scenario per user-facing behavior the PR adds or changes. Each criterion must be phrased as *what the user receives*, from their seat — not what the system did internally. Save them (a scratch file or the message). Writing them first is the whole point: it removes the post-hoc rationalization that lets a failing result get graded ✅.
   ...
   - ❌ "POST returns 200." ❌ "the context payload contains active_document_id." ❌ "a bubble exists." ❌ "the mirrored file is byte-identical / `200` + `image/jpeg`." (All transport — necessary, not sufficient. A checksum or content-type match proves the bytes moved, not that the artifact is the *right* artifact.)
   - **Use a REAL representative input, not synthetic filler.** Eval a real record / real document / real asset when one is reasonably obtainable — `picsum`, `lorem ipsum`, `foo@test.com`, an all-zeros blob can pass an eval the real input would fail (wrong format, size, encoding, content), and they leave junk in real buckets/DBs. If only synthetic input is feasible, say so and why.
...
5. **Visual pass — actually look.** Take screenshots (or read rendered `textContent`, not your own boolean asserts) of every changed surface. Look for: raw markdown, overlapping/stuck overlays, broken layout, truncated/garbled text. **Check the real viewport** — if the product is used at desktop width, eval at desktop width (a mobile-width harness can silently turn a side panel into a full-screen dialog and hide the surface under test). **This applies to any file/binary the change produces or moves, too** — open the image, play the clip, open the PDF, and describe what it actually *depicts*; a hash/size/`Content-Type` check is not a look. (Real case: a file-mirroring change was graded ✅ on `200` + bytes-match over a random stock-photo filler nobody had opened — the round-trip was sound, the artifact was junk left in a real bucket.)
